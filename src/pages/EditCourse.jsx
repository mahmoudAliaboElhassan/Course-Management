...
const EditCourse = () => {
  ...
  useEffect(() => {
    ...
    if (courseToEdit?.image) {
      setPreviewImage(courseToEdit.image);
    }
  }, [id, navigate]);
  ...
  const formik = useFormik({
    ...
    enableReinitialize: true, // To ensure form updates when course data changes
    onSubmit: (values) => {
      const reader = new FileReader();
      if (values.image && values.image !== course?.image) {
  ...
  if (!course) {
    return <div>Loading...</div>; // Optional loading state while fetching course
  }
